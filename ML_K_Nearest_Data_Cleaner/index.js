...
function setup() {
    ...
    console.log("Her er dine rene data (Højreklik på objektet -> 'Store object as global variable' for at gemme):")
    console.log(cleanedData)

    // Giv besked på skærmen og lav download knap
    select('#status').html(`Klar! Processerede ${cleanedData.length} rækker.`)
    
    let btn = createButton('Download data.js')
    btn.addClass('button-primary') // Skeleton CSS styling
    btn.mousePressed(() => {
        // Vi laver en nydelig JS fil med data i en variabel
        let content = `const data = ${JSON.stringify(cleanedData, null, 2)}`
        saveStrings([content], 'data.js')
    })
}